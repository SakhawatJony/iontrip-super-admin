...
export default function RefundRejectDialog({ open, onClose, bookingId, token, onSuccess }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const response = await axios.patch(
        `${API_BASE_URL}${API_ENDPOINTS.REFUND_REJECT(id)}`,
      ...
      const apiMessage =
        ...
        err?.message ??
        "Failed to reject refund request. Please try again.";
      ...
      <DialogContent>
        ...
        <Box>
          ...
          <TextField
            ...
            placeholder="Non-refundable fare per airline policy."
            sx={inputSx}
          />
        </Box>
      </DialogContent>
      <DialogActions sx={{ px: 3, pb: 2.5, pt: 0 }}>
        ...
        <Button
          ...
          sx={{
            ...
            boxShadow: "none",
            "&:hover": { bgcolor: "var(--primary-color, #0F2F56)", opacity: 0.9 },
          }}
        >
          {submitting ? "Submitting..." : "Reject Refund"}
        </Button>
      </DialogActions>
    </Dialog>
  );
