...
export default function AddTour() {
  ...
  const [tourFor, setTourFor] = useState("");
  ...
  const handleSubmit = (e) => {
    ...
    const newItem = {
      id: Date.now(),
      // Keep the keys that `AllTour` expects in the table.
      tourType: trimmed.tourType,
      duration: trimmed.duration,
      maxStay: trimmed.maxStay,
      cost: trimmed.cost,
      createdAt: new Date().toISOString(),
      // Store rest too (not shown in table)
      ...trimmed,
    };

    writeItems([newItem, ...items]);
    toast.success("Tour added successfully");
    navigate("/dashboard/tour/alltour");
  };
        ...
        <Grid container spacing={3}>
          ...
          <Grid item xs={12} md={4}>
            ...
            >
              ...
              <MenuItem value="group">Group Tour</MenuItem>
              <MenuItem value="custom">Custom Tour</MenuItem>
            </TextField>
          </Grid>
        ...
        <Grid container spacing={3}>
          <Grid item xs={12} md={6}>
            <TextField
              ...
              variant="outlined"
              fullWidth
              label="Select Tour For"
              value={tourFor}
              onChange={(e) => setTourFor(e.target.value)}
              InputLabelProps={{ shrink: true }}
              sx={fieldSx}
              SelectProps={{ displayEmpty: true }}
            >
